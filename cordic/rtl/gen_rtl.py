...
if __name__ == "__main__":
    ...
    if num_terms >= 32:
        print("Number of terms must be < 32, since we are using 32 bit numbers")
        exit(2)
    ...
    def gen_cordic_blocks(num_terms, prefix="    "):
        ...
        for i in range(num_terms):
            ...
            blocks += f"{prefix}cordic_block c{i}(.x_in(x[{i}]), .y_in(y[{i}]), .theta_in(t[{i}]), .atan(atan[{i}]), .i({int_to_bin(i, 5)}), .x_out({x_out}), .y_out({y_out}), .theta_out(t[{i+1}]));\n"
        return blocks[:-1]
